...
public class ScrapeAway {
    ...
    public Vector<JourneyStats> ListGPXdata() {
        ...
        for (String fn : Filenames) {
            ...
            if (ReadGPX.LoggedPositions.isEmpty()) {
                System.err.println("no data for that date / time in " + fn);
            } else {
    ...
    private JourneyStats CalculateTrackStatistics(Vector<PositionSample> TheData) {
        ...
        double timeMoving = 0; // in seconds
        double timeStopped = 0; // in seconds
        final double movingSpeedThreshold = 1.0; // m/s
        int j = 0;
        for (int i = 1; i < TheData.size(); i++) {
            ...
            if (TheData.get(i).speed < movingSpeedThreshold) {
                timeStopped += sWeight;
            } else {
                timeMoving += sWeight;
            }
        ...
        JS.TimeMoving = timeMoving;
        JS.TimeStopped = timeStopped;
